'''
Template for aerodynamic optimization with ML model and CFD verification.
'''
...
from pygeo import DVConstraints, DVGeometryCustom
...
from .mesh_cfd import surface_meshing, SurfaceMeshingParams
...
if cfd_frequency > 0:
    generators = [surface_meshing, surface_meshing_ML]
    generator_configs = [SurfaceMeshingParams(tip_path=os.path.join(source_dir, "original_tip.xyz")), SurfaceMeshingParams_ML()]
else:
    generators = [surface_meshing_ML]
    generator_configs = [SurfaceMeshingParams_ML()]

DVGeo = DVGeometryCustom(os.path.join(source_dir, "input.json"), generator=generators, generator_config=generator_configs)

DVGeo.addVariable("twists", lower=-4 * np.ones(6), upper=np.zeros(6), scale=0.1, dh=0.0001)

DVGeo.addVariable("cst_u", lower=0. * np.ones(70), upper=0.4 * np.ones(70), scale=1.0, dh=0.0001)
DVGeo.addVariable("cst_l", lower=-0.4 * np.ones(63), upper=0.2 * np.ones(63), scale=1.0, dh=0.0001)
# Add DVGeo object to CFD solver
mlSolver.setDVGeo(DVGeo, useBaseline=len(generators)-1)
