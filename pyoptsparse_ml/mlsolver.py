...
class AeroModel():
    '''
    a template for the machine learning model that could be used in the MLSolver class
    '''
    ...
    def __init__(self) -> None:
        ...
        self.ap = None
        self.last_surface_outputs = None
...
class MLSolver(AeroSolver):
    ...
    def __init__(
        ...
        default_options = {
            ...
            "sens_mode": (str, "BP"),  # BP or FD for evalFunctionsSens
            "fd_step": (float, 1e-5),  # FD step when sens_mode=FD -> suppose the magnitude of the shape is around 1.0
        }
        ...
        self._sens_call_counter = 0
        self._fd_eval_counter = 0

        # print all options
        if self.comm is not None and self.comm.rank == 0:
            print('MLSolver is setting up with options:')
            print('options', self.options)
            if self.getOption("sens_mode") == "FD":
                self.cnd_fd_step = np.array([self.getOption("fd_step") * 0.5 * (self.condition_keys[key][0] + self.condition_keys[key][1]) for key in self.condition_keys if isinstance(self.condition_keys[key], list)])
                print(f'FD steps: shapes {self.getOption("fd_step")}; conditions: {self.cnd_fd_step}')

    # ------------------------------------------------------------------ #
    # Interface setup helpers
    # ------------------------------------------------------------------ #
    def setModel(self, model: AeroModel, ap: AeroProblem):
        '''
    ...
    def _evaluate_model(self, surface_points, condition_vector, forward_only: bool = False):
        '''

        TODO: Consider multicondition optimization, the `values` output can have a batch dimension 
        to enable the model batch process results under multiple operating conditions

        When ``forward_only`` is True (FD sensitivity mode), run under ``torch.no_grad()`` and skip
        autograd so perturbation loops do not build graphs or backprop.
        '''
        if self.comm is not None and self.comm.rank != 0:
            return {
                "values": None,
        ...
        if forward_only:
            with torch.no_grad():
                inp = torch.tensor(surface_points, dtype=torch.float32, device=self.device).unsqueeze(0)
                cnd = torch.tensor(condition_vector, dtype=torch.float32, device=self.device).unsqueeze(0)
        ...
        if self.debug: print(f'ML model gives results {values_np} for input condition {cnd}')
    ...
    def __call__(self, aeroProblem):
        ...
        forward_only = str(self.getOption("sens_mode")).upper() == "FD"
        if self.comm.rank == 0:
            eval_result = self._evaluate_model(self.surface_mesh, condition_vector, forward_only=forward_only)
        else:
            eval_result = None
        ...
        if not forward_only and (
            any(np.isnan(arr).any() for arr in self._last_surface_grad_global.values())
            or any(np.isnan(arr).any() for arr in self._last_condition_grad.values())
        ):
    ...
    def evalFunctionsSens(self, aeroProblem, funcsSens, evalFuncs=None, ignoreMissing=True):
        ...
        t0 = time.time()
        self._sens_call_counter += 1
        sens_mode = str(self.getOption("sens_mode")).upper()
        if sens_mode not in {"BP", "FD"}:
            raise ValueError(f"Invalid MLSolver sens_mode '{sens_mode}'. Use 'BP' or 'FD'.")
        surf_grad_map = None
        cond_grad_map = None
        condition_meta = self._last_condition_meta

        if sens_mode == "FD":
            fd_step = float(self.getOption("fd_step"))
            if fd_step <= 0.0:
                raise ValueError("MLSolver option 'fd_step' must be > 0.")

            out_of_range, condition_vector, condition_meta = self._build_condition_vector(aeroProblem)
            # if out_of_range:
            #     raise RuntimeError("MLSolver FD sensitivity requested with out-of-range condition vector.")
            surface_base = np.array(self.surface_mesh, copy=True, dtype=float)
            condition_base = np.array(condition_vector, copy=True, dtype=float)

            def _eval_outputs(surface_points: np.ndarray, condition_vec: np.ndarray) -> Dict[str, float]:
                eval_result = self._evaluate_model(surface_points, condition_vec, forward_only=True)
                self._fd_eval_counter += 1
                return {k.lower(): float(v) for k, v in eval_result["values"].items()}

            if self.comm is None or self.comm.rank == 0:
                base_vals = _eval_outputs(surface_base, condition_base)
                surf_grad_map = {k: np.zeros_like(surface_base, dtype=float) for k in base_vals.keys()}
                cond_grad_map = {k: np.zeros_like(condition_base, dtype=float) for k in base_vals.keys()}

                surf_flat = surface_base.reshape(-1)
                for i in range(surf_flat.size):
                    print(f'surface for {i}, fd_step {fd_step}')
                    surf_pert = surface_base.copy().reshape(-1)
                    surf_pert[i] += fd_step
                    vals = _eval_outputs(surf_pert.reshape(surface_base.shape), condition_base)
                    for key in base_vals.keys():
                        surf_grad_map[key].reshape(-1)[i] = (vals[key] - base_vals[key]) / fd_step
                print('surf_grad_map', surf_grad_map)

                for j in range(condition_base.size):
                    cond_pert = condition_base.copy()
                    cond_pert[j] += self.cnd_fd_step[j]
                    vals = _eval_outputs(surface_base, cond_pert)
                    for key in base_vals.keys():
                        cond_grad_map[key][j] = (vals[key] - base_vals[key]) / fd_step
                print('cond_grad_map', cond_grad_map)

                # if self.debug:
                print(
                    f"[MLSolver-FD] step={fd_step:.2e}, surface_dims={surf_flat.size}, "
                    f"cond_dims={condition_base.size}, evals={1 + surf_flat.size + condition_base.size}"
                )
            if self.comm is not None:
                surf_grad_map = self.comm.bcast(surf_grad_map, root=0)
                cond_grad_map = self.comm.bcast(cond_grad_map, root=0)
        else:
            surf_grad_map = self._last_surface_grad_global
            cond_grad_map = self._last_condition_grad
        ...
        for func in evalFuncs:
            ...
            local_grad = surf_grad_map.get(key)
            ...
            condition_grad = cond_grad_map.get(key)
        ...
        if self.comm is None or self.comm.rank == 0:
            ...
            # grad_norms = []
            # for fkey, dct in funcsSens.items():
            #     if isinstance(dct, dict):
            #         sq = 0.0
            #         for v in dct.values():
            #             arr = np.asarray(v, dtype=float)
            #             sq += float(np.dot(arr.reshape(-1), arr.reshape(-1)))
            #         grad_norms.append(f"{fkey}: {sq**0.5:.3e}")
            print(
                f"[MLSolver-Sens] mode={sens_mode}, call={self._sens_call_counter}, "
                f"fd_total_evals={self._fd_eval_counter}, elapsed={elapsed:.2f}s"
            )
            # if grad_norms:
            #     print("[MLSolver-Sens] grad norms -> " + ", ".join(grad_norms))
